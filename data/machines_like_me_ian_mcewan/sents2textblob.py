...
with open("mlm_sentences.txt", "r+") as fp_in:
    for i, aline in enumerate(fp_in):
        ...
        if len(aline_str) == 0:
            ...
            tb_polarity = TextBlob(sent_clean_str).polarity
            tb_subjectivity = TextBlob(sent_clean_str).subjectivity
            
            # Format to 2 decimal places
            tb_polarity_rounded = f'{tb_polarity:.2f}'
